# recepzerk.github.io

A hub for interactive tools built by Recep Zerk, Digital Literacy Advocate and ultramarathon runner.

🔗 **[Visit the tools hub](https://recepzerk.github.io)**

## What's here

**Digital Literacy Tools** — Self-assessment quizzes and calculators on digital wellbeing, algorithm awareness, and screen habits. Based on independent research published on Zenodo.

**Running Tools** — Practical tools for trail and road runners. Weather-based outfit recommendations, pace-matched playlists, GPX route analysis, and more.

All tools are free, open, and require no signup.

## Links

- [recepzerk.com](https://recepzerk.com)
- [Research on Zenodo](https://zenodo.org/records/20149107)
- [GitHub Profile](https://github.com/recepzerk)
